...
int Disassemble8080Op(std::vector<unsigned char> &codebuffer, int pc)
{
...
int main(int argc, char **argv)
{
    try
    {
        ...
        std::ifstream rfile(argv[1], std::ios::binary);

        if (!rfile.is_open())
        {
            throw std::runtime_error("Error: Couldn't open file " + std::string(argv[1]));
        }

        // Get the file size
        rfile.seekg(0, std::ios::end);
        std::streamsize fsize = rfile.tellg();
        rfile.seekg(0, std::ios::beg);

        if (fsize < 0)
        {
            throw std::runtime_error("Error: Unable to determine file size");
        }

        // Read the file content into a vector
        std::vector<unsigned char> buffer(fsize);
        if (!rfile.read(reinterpret_cast<char *>(buffer.data()), fsize))
        {
            throw std::runtime_error("Error: Failed to read file");
        }

        // Close the file explicitly (optional since the destructor will close it)
        rfile.close();

        // Process the data using Disassemble8080Op
        int pc = 0;
        while (pc < fsize)
        {
